...
//        });
        ...
        GatewayFilter filter = new OrderedGatewayFilter((exchange, chain) -> {
            ...
            log.info("Custom Logging Filter baseMessage: {}", config.getBaseMessage());
            if (config.isPreLogger()) {
                log.info("Custom Logging PRE Filter: request id -> {}", request.getId());
            }
            return chain.filter(exchange).then(Mono.fromRunnable(()->{
                if (config.isPostLogger()) {
                    log.info("Custom Logging POST Filter: response code -> {}", response.getStatusCode());
                }
            }));
